...
| `block_groups` | follower | per-block aggregate, one row per (to, selector, bucket, status) with its tx count and the block time. The rolling 1h / 24h / 7d stats are sums over this table |
