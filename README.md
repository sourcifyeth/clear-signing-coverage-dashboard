...
| `window_groups`, `window_meta` | follower | running totals per rolling window (1h / 24h / 7d): one row per (to, selector, bucket, status) summed over the blocks the window includes, plus the block range and totals. Updated with every block (add the new one, subtract the expired ones), rebuilt at follower start. `npm run windows:check` compares them with a fresh computation |
